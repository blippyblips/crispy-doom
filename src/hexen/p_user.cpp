...
void P_MovePlayer(player_t * player)
{
    ...
    {
        if (player->mo->state == &states[PStateNormal[player->_class]])
        {
            P_SetMobjState(player->mo, static_cast<statenum_t>(PStateRun[player->_class]));
        }
    }
...
void P_DeathThink(player_t * player)
{
    ...
    if (player->cmd.buttons & BT_USE)
    {
        ...
        player->mo->special1.i = player->_class;
...
boolean P_UndoPlayerMorph(player_t * player)
{
    ...
    if (player->_class == PCLASS_FIGHTER)
    {
...
//              go away, even in death.
    ...
    else
    {
            ...
            && P_AproxDistance(pmo->momx, pmo->momy) > 12 * FRACUNIT)
        {
            ...
            if (speedMo)
            {
                ...
                speedMo->target = pmo;
                speedMo->special1.i = player->_class;
    ...
    {
        if (player->_class == PCLASS_CLERIC)
        {
...
boolean P_UseArtifact(player_t * player, artitype_t arti)
{
    ...
    switch (arti)
    {
        ...
        case arti_poisonbag:
            ...
            else if (player->_class == PCLASS_MAGE)
            {
                mo = P_SpawnMobj(player->mo->x + 16 * finecosine[angle],
                                 player->mo->y + 24 * finesine[angle],
                                 player->mo->z - player->mo->floorclip +
