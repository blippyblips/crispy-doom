...
//
    ...
    if (thing->flags & MF_TRANSLATION)
    {
        if (thing->player)
        {
            vis->_class = thing->player->_class;
        }
        else
        {
            vis->_class = thing->special1.i;
        }
        if (vis->_class > 2)
        {
            vis->_class = 0;
...
//
    ...
    {
        vis->texturemid -= PSpriteSY[viewplayer->_class]
    ...
    if (viewplayer->powers[pw_invulnerability] && viewplayer->_class
        == PCLASS_CLERIC)
    {
