...
static void StreamIn_player_t(player_t *str)
{
    ...
    // pclass_t class;
    str->_class = static_cast<pclass_t>(SV_ReadLong());
...
static void StreamIn_thinker_t(thinker_t *str)
{
    ...
    str->prev = static_cast<thinker_t*>(SV_ReadPtr());
    str->prev = NULL;
    str->next = static_cast<thinker_t*>(SV_ReadPtr());
    str->next = NULL;

    // think_t function;
    // Function pointer is discarded:
    str->function = static_cast<think_t>(SV_ReadPtr());
    str->function = NULL;
}
...
static void StreamIn_mobj_t(mobj_t *str)
{
    ...
    str->snext = static_cast<mobj_t*>(SV_ReadPtr());
    str->snext = NULL;
    str->sprev = static_cast<mobj_t*>(SV_ReadPtr());
    str->sprev = NULL;
    ...
    // position is set.
    str->bnext = static_cast<mobj_t*>(SV_ReadPtr());
    str->bnext = NULL;
    str->bprev = static_cast<mobj_t*>(SV_ReadPtr());
    str->bprev = NULL;

    // subsector_t* subsector;
    // Read but discard: pointer will be restored when thing position is set.
    str->subsector = static_cast<subsector_t*>(SV_ReadPtr());
    str->subsector = NULL;
...
void SV_LoadGame(int slot)
{
    ...
    for (i = 0; i < p->inventorySlotNum; i++)
    {
        if (p->inventory[i].type == p->readyArtifact)
        {
            ...
            p->readyArtifact = static_cast<artitype_t>(p->inventory[inv_ptr].type);
            break;
        }
    }
...
static void UnarchiveMobjs(void)
{
    ...
    TargetPlayerAddrs = static_cast<mobj_t***>(Z_Malloc(MAX_TARGET_PLAYERS * sizeof(mobj_t **), PU_STATIC, NULL));
    TargetPlayerCount = 0;
