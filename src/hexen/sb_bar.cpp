...
void SB_Init(void)
{
    ...
    PatchH2BAR = static_cast<patch_t*>(W_CacheLumpName("H2BAR", PU_STATIC));
    PatchH2TOP = static_cast<patch_t*>(W_CacheLumpName("H2TOP", PU_STATIC));
    PatchINVBAR = static_cast<patch_t*>(W_CacheLumpName("INVBAR", PU_STATIC));
    PatchLFEDGE = static_cast<patch_t*>(W_CacheLumpName("LFEDGE", PU_STATIC));
    PatchRTEDGE = static_cast<patch_t*>(W_CacheLumpName("RTEDGE", PU_STATIC));
    PatchSTATBAR = static_cast<patch_t*>(W_CacheLumpName("STATBAR", PU_STATIC));
    PatchKEYBAR = static_cast<patch_t*>(W_CacheLumpName("KEYBAR", PU_STATIC));
    PatchSELECTBOX = static_cast<patch_t*>(W_CacheLumpName("SELECTBOX", PU_STATIC));
    PatchARTICLEAR = static_cast<patch_t*>(W_CacheLumpName("ARTICLS", PU_STATIC));
    PatchARMCLEAR = static_cast<patch_t*>(W_CacheLumpName("ARMCLS", PU_STATIC));
    PatchMANACLEAR = static_cast<patch_t*>(W_CacheLumpName("MANACLS", PU_STATIC));
    PatchMANAVIAL1 = static_cast<patch_t*>(W_CacheLumpName("MANAVL1", PU_STATIC));
    PatchMANAVIAL2 = static_cast<patch_t*>(W_CacheLumpName("MANAVL2", PU_STATIC));
    PatchMANAVIALDIM1 = static_cast<patch_t*>(W_CacheLumpName("MANAVL1D", PU_STATIC));
    PatchMANAVIALDIM2 = static_cast<patch_t*>(W_CacheLumpName("MANAVL2D", PU_STATIC));
    PatchMANADIM1 = static_cast<patch_t*>(W_CacheLumpName("MANADIM1", PU_STATIC));
    PatchMANADIM2 = static_cast<patch_t*>(W_CacheLumpName("MANADIM2", PU_STATIC));
    PatchMANABRIGHT1 = static_cast<patch_t*>(W_CacheLumpName("MANABRT1", PU_STATIC));
    PatchMANABRIGHT2 = static_cast<patch_t*>(W_CacheLumpName("MANABRT2", PU_STATIC));
    PatchINVLFGEM1 = static_cast<patch_t*>(W_CacheLumpName("invgeml1", PU_STATIC));
    PatchINVLFGEM2 = static_cast<patch_t*>(W_CacheLumpName("invgeml2", PU_STATIC));
    PatchINVRTGEM1 = static_cast<patch_t*>(W_CacheLumpName("invgemr1", PU_STATIC));
    PatchINVRTGEM2 = static_cast<patch_t*>(W_CacheLumpName("invgemr2", PU_STATIC));
...
//      PatchCHAINBACK = W_CacheLumpName("CHAINBACK", PU_STATIC);
    ...
    PatchNEGATIVE = static_cast<patch_t*>(W_CacheLumpName("NEGNUM", PU_STATIC));
    ...
    if (deathmatch)
    {
        PatchKILLS = static_cast<patch_t*>(W_CacheLumpName("KILLS", PU_STATIC));
    }
    SB_SetClassData();
...
{
    int _class = PlayerClass[consoleplayer]; // original player class (not pig)
    PatchWEAPONSLOT = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("wpslot0") + _class, PU_STATIC));
    PatchWEAPONFULL = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("wpfull0") + _class, PU_STATIC));
    PatchPIECE1 = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("wpiecef1") + _class, PU_STATIC));
    PatchPIECE2 = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("wpiecef2") + _class, PU_STATIC));
    PatchPIECE3 = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("wpiecef3") + _class, PU_STATIC));
    PatchCHAIN = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("chain") + _class, PU_STATIC));
    if (!netgame)
    {                           // single player game uses red life gem (the second gem)
        PatchLIFEGEM = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("lifegem")
                                      + maxplayers * _class + 1, PU_STATIC));
    }
    else
    {
        PatchLIFEGEM = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("lifegem")
                                      + maxplayers * _class + consoleplayer,
                                      PU_STATIC));
    }
    SB_state = -1;
...
static void RefreshBackground(void)
{
    ...
    if ((SCREENWIDTH >> crispy->hires) != ORIGWIDTH)
    {
        ...
        src = static_cast<byte*>(W_CacheLumpName("F_022", PU_CACHE));
        ...
        {
            int x;
            patch_t *const patch = static_cast<patch_t*>(W_CacheLumpName("bordb", PU_CACHE));
    ...
    if (CPlayer->powers[pw_flight])
    {
            ...
            || !(CPlayer->powers[pw_flight] & 16))
        {
            ...
            if (CPlayer->mo->flags2 & MF2_FLY)
            {
                if (hitCenterFrame && (frame != 15 && frame != 0))
                {
                    V_DrawPatch(spinfly_x, 19,
                                static_cast<patch_t*>(W_CacheLumpNum(SpinFlylump + 15, PU_CACHE)));
                }
                else
                {
                    V_DrawPatch(spinfly_x, 19, static_cast<patch_t*>(W_CacheLumpNum(SpinFlylump + frame, PU_CACHE)));
            ...
            else
            {
                ...
                {
                    V_DrawPatch(spinfly_x, 19,
                                static_cast<patch_t*>(W_CacheLumpNum(SpinFlylump + frame, PU_CACHE)));
                    hitCenterFrame = false;
                }
                else
                {
    ...
    if (CPlayer->powers[pw_speed])
    {
            ...
            || !(CPlayer->powers[pw_speed] & 16))
        {
            ...
            V_DrawPatch(spinspeed_x, 19,
                        static_cast<patch_t*>(W_CacheLumpNum(SpinSpeedLump + frame,
                                        PU_CACHE)));
        }
        BorderTopRefresh = true;
        UpdateState |= I_MESSAGES;
    ...
    if (CPlayer->powers[pw_invulnerability])
    {
            ...
            || !(CPlayer->powers[pw_invulnerability] & 16))
        {
            ...
            V_DrawPatch(spindefense_x, 19,
                        static_cast<patch_t*>(W_CacheLumpNum(SpinDefenseLump + frame, PU_CACHE)));
        }
        BorderTopRefresh = true;
        UpdateState |= I_MESSAGES;
    }
    ...
    if (CPlayer->powers[pw_minotaur])
    {
            ...
            || !(CPlayer->powers[pw_minotaur] & 16))
        {
            ...
            V_DrawPatch(spinminotaur_x, 19,
                        static_cast<patch_t*>(W_CacheLumpNum(SpinMinotaurLump + frame, PU_CACHE)));
        }
        BorderTopRefresh = true;
        UpdateState |= I_MESSAGES;
    }
...
void DrawMainBar(void)
{
    ...
    // Armor
    temp = AutoArmorSave[CPlayer->_class]
...
void DrawKeyBar(void)
{
        ...
        for (i = 0; i < NUM_KEY_TYPES && xPosition <= 126; i++)
        {
            if (CPlayer->keys & (1 << i))
            {
                V_DrawPatch(xPosition, 164,
                            static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("keyslot1") + i, PU_CACHE)));
                xPosition += 20;
            }
        }
    ...
    if (oldarmor != temp)
    {
        for (i = 0; i < NUMARMOR; i++)
        {
            ...
            if (CPlayer->armorpoints[i] <=
                (ArmorIncrement[CPlayer->_class][i] >> 2))
            {
                V_DrawTLPatch(150 + 31 * i, 164,
                              static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName("armslot1") + i, PU_CACHE)));
            }
            else if (CPlayer->armorpoints[i] <=
                     (ArmorIncrement[CPlayer->_class][i] >> 1))
            {
...
void DrawFullScreenStuff(void)
{
    ...
    if (!inventory)
    {
        if (CPlayer->readyArtifact > 0)
        {
            V_DrawTLPatch(286, 170, static_cast<patch_t*>(W_CacheLumpName("ARTIBOX", PU_CACHE)));
            V_DrawPatch(284, 169, static_cast<patch_t*>(W_CacheLumpName(patcharti[CPlayer->readyArtifact], PU_CACHE)));
            if (CPlayer->inventory[inv_ptr].count > 1)
            {
    ...
    else
    {
        ...
        for (i = 0; i < 7; i++)
        {
            V_DrawTLPatch(50 + i * 31, 168, static_cast<patch_t*>(W_CacheLumpName("ARTIBOX", PU_CACHE)));
            if (CPlayer->inventorySlotNum > x + i
                && CPlayer->inventory[x + i].type != arti_none)
            {
...
static void CheatWeaponsFunc(player_t * player, Cheat_t * cheat)
{
    ...
    for (i = 0; i < NUMARMOR; i++)
    {
        player->armorpoints[i] = ArmorIncrement[player->_class][i];
    }
...
static void CheatClassFunc2(player_t * player, Cheat_t * cheat)
{
    ...
    _class = static_cast<pclass_t>(args[0] - '0');
    if (_class > 2 || _class < 0)
    {
        P_SetMessage(player, "INVALID PLAYER CLASS", true);
        return;
    }
    player->_class = static_cast<pclass_t>(_class);
    for (i = 0; i < NUMARMOR; i++)
    {
        player->armorpoints[i] = 0;
    }
    PlayerClass[consoleplayer] = static_cast<pclass_t>(_class);
