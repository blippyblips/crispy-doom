...
void P_PostMorphWeapon(player_t * player, weapontype_t weapon)
{
    ...
    P_SetPsprite(player, ps_weapon,
                 WeaponInfo[weapon][player->_class].upstate);
}
...
void P_BringUpWeapon(player_t * player)
{
    ...
    if (player->_class == PCLASS_FIGHTER && player->pendingweapon == WP_SECOND
        && player->mana[MANA_1])
    {
        new_state = S_FAXEUP_G;
    }
    else
    {
        new_state = WeaponInfo[player->pendingweapon][player->_class].upstate;
    }
    player->pendingweapon = WP_NOCHANGE;
    player->psprites[ps_weapon].sy = WEAPONBOTTOM;
...
boolean P_CheckMana(player_t * player)
{
    ...
    mana = WeaponInfo[player->readyweapon][player->_class].mana;
    count = WeaponManaUse[player->_class][player->readyweapon];
    if (mana == MANA_BOTH)
    {
    ...
    P_SetPsprite(player, ps_weapon,
                 WeaponInfo[player->readyweapon][player->_class].downstate);
    return (false);
}
...
void P_FireWeapon(player_t * player)
{
    ...
    P_SetMobjState(player->mo, static_cast<statenum_t>(PStateAttack[player->_class]));    // S_PLAY_ATK1);
    if (player->_class == PCLASS_FIGHTER && player->readyweapon == WP_SECOND
    ...
    else
    {
        attackState = static_cast<statenum_t>(player->refire ?
            WeaponInfo[player->readyweapon][player->_class].holdatkstate
            : WeaponInfo[player->readyweapon][player->_class].atkstate);
    }
...
void A_WeaponReady(mobj_t *obj, player_t *player, pspdef_t *psp)
{
    ...
    if (player->mo->state >= &states[PStateAttack[player->_class]]
        && player->mo->state <= &states[PStateAttackEnd[player->_class]])
    {
        P_SetMobjState(player->mo, static_cast<statenum_t>(PStateNormal[player->_class]));
    }
    ...
    {
        P_SetPsprite(player, ps_weapon,
                     WeaponInfo[player->readyweapon][player->_class].
                     downstate);
        return;
    }
...
void A_FHammerAttack(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
  ...
  hammerdone:
    if (player->mana[MANA_2] <
        WeaponManaUse[player->_class][player->readyweapon])
...
void A_FHammerThrow(mobj_t *mobj, player_t * player, pspdef_t * psp)
{
    ...
    player->mana[MANA_2] -= WeaponManaUse[player->_class][player->readyweapon];
...
void A_FSwordAttack(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
    ...
    player->mana[MANA_1] -= WeaponManaUse[player->_class][player->readyweapon];
    player->mana[MANA_2] -= WeaponManaUse[player->_class][player->readyweapon];
    pmo = player->mo;
...
void P_MovePsprites(player_t * player)
{
    ...
    if (psp->state && (crispy->bobfactor || crispy->centerweapon || crispy->uncapped))
    {
        ...
        const boolean swiping_weapon = player->_class == PCLASS_FIGHTER ||
                                       (player->_class == PCLASS_CLERIC &&
