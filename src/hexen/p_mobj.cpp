...
void P_SpawnPlayer(mapthing_t * mthing)
{
    ...
    {
        p->_class = static_cast<pclass_t>(P_Random() % 3);
        if (p->_class == PlayerClass[mthing->type - 1])
        {
            p->_class = static_cast<pclass_t>((p->_class + 1) % 3);
        }
        PlayerClass[mthing->type - 1] = p->_class;
        SB_SetClassData();
    }
    else
    {
        p->_class = PlayerClass[mthing->type - 1];
    }
    switch (p->_class)
    {
        case PCLASS_FIGHTER:
