...
static void DrawMainMenu(void)
{
    ...
    frame = (MenuTime / 5) % 7;
    V_DrawPatch(88, 0, static_cast<patch_t*>(W_CacheLumpName("M_HTIC", PU_CACHE)));
...
static void DrawClassMenu(void)
{
    ...
    _class = (pclass_t) CurrentMenu->items[CurrentItPos].option;
    V_DrawPatch(174, 8, static_cast<patch_t*>(W_CacheLumpName(boxLumpName[_class], PU_CACHE)));
    V_DrawPatch(174 + 24, 8 + 12,
                static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName(walkLumpName[_class])
...
static void DrawSaveLoadBottomLine(const Menu_t *menu)
{
    ...
    if (!width)
    {
        const patch_t *const p = static_cast<patch_t*>(W_CacheLumpName("M_FSLOT", PU_CACHE));
        width = SHORT(p->width);
    }
...
static void DrawFileSlots(Menu_t * menu)
{
    ...
    {
        V_DrawPatch(x, y, static_cast<patch_t*>(W_CacheLumpName("M_FSLOT", PU_CACHE)));
        if (SlotStatus[i])
        {
...
static void DrawSlider(Menu_t * menu, int item, int width, int slot)
{
    ...
    {
        V_DrawPatch(x2, y, static_cast<patch_t*>(W_CacheLumpName(count & 1 ? "M_SLDMD1" : "M_SLDMD2", PU_CACHE)));
    }
    ...
    V_DrawPatch(x + 4 + slot * 8, y + 7,
                static_cast<patch_t*>(W_CacheLumpName("M_SLDKB", PU_CACHE)));
}
