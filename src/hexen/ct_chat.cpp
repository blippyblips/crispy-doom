...
void CT_Drawer(void)
{
    ...
    if (chatmodeon)
    {
        ...
        for (i = 0; i < msgptr[consoleplayer]; i++)
        {
            ...
            {
                patch = static_cast<patch_t*>(W_CacheLumpNum(FontABaseLump + chat_msg[consoleplayer][i] - 33, PU_CACHE));
                V_DrawPatch(x, 10, patch);
                x += patch->width;
            }
        }
        V_DrawPatch(x, 10, static_cast<patch_t*>(W_CacheLumpName("FONTA59", PU_CACHE)));
