...
void D_DoomMain(void)
{
    ...
    startskill = static_cast<skill_t>((crispy->defaultskill + SKILL_HMP) % NUM_SKILLS);
...
static void HandleArgs(void)
{
    ...
    if (p)
    {
        ...
        uc_filename = _strdup(myargv[p + 1]);
...
static void DrawAndBlit(void)
{
    ...
    if (paused && !MenuActive && !askforquit)
    {
        if (!netgame)
        {
            V_DrawPatch(160, (viewwindowy >> crispy->hires) + 5, static_cast<patch_t*>(W_CacheLumpName("PAUSED", PU_CACHE)));
        }
        else
        {
            V_DrawPatch(160, 70, static_cast<patch_t*>(W_CacheLumpName("PAUSED", PU_CACHE)));
        }
    }
...
static void PageDrawer(void)
{
    ...
    if (demosequence == 1)
    {
        V_DrawPatch(4, 160, static_cast<patch_t*>(W_CacheLumpName("ADVISOR", PU_CACHE)));
    }
    UpdateState |= I_FULLSCRN;
}
