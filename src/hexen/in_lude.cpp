...
static void LoadPics(void)
{
    ...
    {
        patchINTERPIC = static_cast<patch_t*>(W_CacheLumpName("INTERPIC", PU_STATIC));
        FontBLumpBase = W_GetNumForName("FONTB16");
        for (i = 0; i < 10; i++)
        {
            FontBNumbers[i] = static_cast<patch_t*>(W_CacheLumpNum(FontBLumpBase + i, PU_STATIC));
        }
        FontBLump = W_GetNumForName("FONTB_S") + 1;
        FontBNegative = static_cast<patch_t*>(W_CacheLumpName("FONTB13", PU_STATIC));
        FontABaseLump = W_GetNumForName("FONTA_S") + 1;

        FontBSlash = static_cast<patch_t*>(W_CacheLumpName("FONTB15", PU_STATIC));
        FontBPercent = static_cast<patch_t*>(W_CacheLumpName("FONTB05", PU_STATIC));
    }
}
...
static void DrDeathTally(void)
{
    ...
    V_DrawPatch(TALLY_TOP_X, TALLY_TOP_Y,
                static_cast<patch_t*>(W_CacheLumpName("tallytop", PU_CACHE)));
    V_DrawPatch(TALLY_LEFT_X, TALLY_LEFT_Y,
                static_cast<patch_t*>(W_CacheLumpName("tallylft", PU_CACHE)));
