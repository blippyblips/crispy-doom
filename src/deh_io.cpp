...
static deh_context_t *DEH_NewContext(void)
{
    ...
    context = static_cast<deh_context_t*>(Z_Malloc(sizeof(*context), PU_STATIC, NULL));

    // Initial read buffer size of 128 bytes

    context->readbuffer_size = 128;
    context->readbuffer = static_cast<char*>(Z_Malloc(context->readbuffer_size, PU_STATIC, NULL));
    context->linenum = 0;
...
deh_context_t *DEH_OpenLump(int lumpnum)
{
    ...
    context->input_buffer = static_cast<unsigned char*>(lump);
...
static void IncreaseReadBuffer(deh_context_t *context)
{
    ...
    newbuffer = static_cast<char*>(Z_Malloc(newbuffer_size, PU_STATIC, NULL));
