...
static void RefreshBackground(void)
{
    ...
    {
        int x;
        patch_t *const patch = static_cast<patch_t*>(W_CacheLumpName(DEH_String("brdr_b"), PU_CACHE));
...
void ST_doRefresh(void)
{
    ...
    if (st_statusbaron)
    {
            ...
            num_x += 35, icon_x += 35, i++, numdrawn++)
        {
            ...
            if(lumpnum == -1)
                patch = static_cast<patch_t*>(W_CacheLumpName(DEH_String("STCFN063"), PU_CACHE));
            else
                patch = static_cast<patch_t*>(W_CacheLumpNum(lumpnum, PU_STATIC));
...
static boolean ST_drawKeysPopup(void)
{
    ...
    if(deathmatch)
    {
        ...
        for(pnum = 0; pnum < MAXPLAYERS/2; pnum++)
        {
            ...
            colpatch = static_cast<patch_t*>(W_CacheLumpName(buffer, PU_CACHE));
    ...
    else
    {
        ...
        for(; keycount < ST_KEYSPERPAGE && key < NUMCARDS; ++key, ++keycount, ++info)
        {
            ...
            if(plyr->cards[key])
            {
                ...
                patch = static_cast<patch_t*>(W_CacheLumpName(sprname, PU_CACHE));
...
static void ST_drawCrispyHUD(void)
{
    ...
    // [crispy] health
    patch = static_cast<patch_t*>(W_CacheLumpName(DEH_String("I_MDKT"), PU_STATIC));
        ...
        ST_drawNumFontY2(261, 132, keys);
         ...
         if(plyr->weaponowned[wp_elecbow])
         {
             V_DrawPatchDirect(38, 86, 
                 static_cast<patch_t*>(W_CacheLumpName(DEH_String("CBOWA0"), PU_CACHE)));
         }
         if(plyr->weaponowned[wp_rifle])
         {
             V_DrawPatchDirect(40, 107, 
                 static_cast<patch_t*>(W_CacheLumpName(DEH_String("RIFLA0"), PU_CACHE)));
         }
         if(plyr->weaponowned[wp_missile])
         {
             V_DrawPatchDirect(39, 131, 
                 static_cast<patch_t*>(W_CacheLumpName(DEH_String("MMSLA0"), PU_CACHE)));
         }
         if(plyr->weaponowned[wp_hegrenade])
         {
             V_DrawPatchDirect(78, 87, 
                 static_cast<patch_t*>(W_CacheLumpName(DEH_String("GRNDA0"), PU_CACHE)));
         }
         if(plyr->weaponowned[wp_flame])
         {
             V_DrawPatchDirect(80, 117, 
                 static_cast<patch_t*>(W_CacheLumpName(DEH_String("FLAMA0"), PU_CACHE)));
         }
         if(plyr->weaponowned[wp_mauler])
         {
             V_DrawPatchDirect(75, 142, 
                 static_cast<patch_t*>(W_CacheLumpName(DEH_String("TRPDA0"), PU_CACHE)));
         }
         ...
         if(plyr->powers[pw_communicator])
         {
             V_DrawPatchDirect(280, 130, 
                 static_cast<patch_t*>(W_CacheLumpName(DEH_String("I_COMM"), PU_CACHE)));
         }
    }
