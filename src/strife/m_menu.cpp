...
{
    int i;
    V_DrawPatchDirect(72, 28, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_LOADG"), PU_CACHE)));
...
void M_DrawReadThis1(void)
{
    ...
    V_DrawPatchFullScreen(static_cast<patch_t*>(W_CacheLumpName(DEH_String("HELP1"), PU_CACHE)), false);
}
...
void M_DrawReadThis2(void)
{
    ...
    V_DrawPatchFullScreen(static_cast<patch_t*>(W_CacheLumpName(DEH_String("HELP2"), PU_CACHE)), false);
}
...
void M_DrawReadThis3(void)
{
    ...
    V_DrawPatchFullScreen(static_cast<patch_t*>(W_CacheLumpName(DEH_String("HELP3"), PU_CACHE)), false);
}
...
void M_DrawMainMenu(void)
{
    V_DrawPatchDirect(84, 2,
                      static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_STRIFE"), PU_CACHE)));
}
...
void M_DrawNewGame(void)
{
    V_DrawPatchDirect(96, 14, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_NGAME"), PU_CACHE)));
...
void M_DrawOptions(void)
{
    ...
    V_DrawPatchDirect(108, 15, 
                      static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_OPTION"), PU_CACHE)));
...
static void M_DrawCrispnessBackground(void)
{
    ...
    src = static_cast<byte*>(W_CacheLumpName(back_flat, PU_CACHE));
    dest = I_VideoBuffer;
...
void M_EndGame(int choice)
{
    ...
    if (netgame)
    {
        M_StartMessage(DEH_String(const_cast<char*>(NETEND)),NULL,false);
        return;
    }

    M_StartMessage(DEH_String(const_cast<char*>(ENDGAME)),M_EndGameResponse,true);
}
  ...
  int	thermDot )
{
    ...
    V_DrawPatchDirect(xx, yy, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_THERML"), PU_CACHE)));
    xx += 8;
    for (i=0;i<thermWidth;i++)
    {
        V_DrawPatchDirect(xx, yy, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_THERMM"), PU_CACHE)));
        xx += 8;
    }
    V_DrawPatchDirect(xx, yy, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_THERMR"), PU_CACHE)));
...
void M_Drawer (void)
{
    ...
    V_DrawPatchDirect(x + CURSORXOFF, currentMenu->y - 5 + itemOn*LINEHEIGHT,
                      static_cast<patch_t*>(W_CacheLumpName(DEH_String(cursorName[whichCursor]),
                                      PU_CACHE)));
    }
}
