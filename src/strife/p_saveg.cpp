...
{
    // actionf_p1 acp1;
    *str = static_cast<actionf_p1>(saveg_readp());
}
...
void P_ArchiveSpecials (void)
{
    ...
    for (th = thinkercap.next ; th != &thinkercap ; th=th->next)
    {
        ...
        if (th->function == (actionf_p1)T_SlidingDoor)
        {
...
void P_UnArchiveSpecials (void)
{
    ...
    while (1)
    {
        ...
        case tc_slidingdoor:
            ...
            saveg_read_pad();
            slidedoor = static_cast<slidedoor_t*>(Z_Malloc(sizeof(*slidedoor), PU_LEVEL, NULL));
            saveg_read_slidedoor_t(slidedoor);
            slidedoor->frontsector->specialdata = slidedoor;
            slidedoor->thinker.function = (actionf_p1)T_SlidingDoor;
