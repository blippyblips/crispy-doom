...
{
    V_DrawPatchFullScreen (static_cast<patch_t*>(W_CacheLumpName(pagename, PU_CACHE)), false);
}
...
static void D_Endoom(void)
{
    ...
    endoom = static_cast<byte*>(W_CacheLumpName(DEH_String("ENDSTRF"), PU_STATIC));
...
static void D_InitIntroSequence(void)
{
    ...
    if(showintro)
    {
        ...
        // Load all graphics
        rawgfx_startup0   = static_cast<byte*>(W_CacheLumpName("STARTUP0", PU_STATIC));
        rawgfx_startp[0]  = static_cast<byte*>(W_CacheLumpName("STRTPA1",  PU_STATIC));
        rawgfx_startp[1]  = static_cast<byte*>(W_CacheLumpName("STRTPB1",  PU_STATIC));
        rawgfx_startp[2]  = static_cast<byte*>(W_CacheLumpName("STRTPC1",  PU_STATIC));
        rawgfx_startp[3]  = static_cast<byte*>(W_CacheLumpName("STRTPD1",  PU_STATIC));
        rawgfx_startlz[0] = static_cast<byte*>(W_CacheLumpName("STRTLZ1",  PU_STATIC));
        rawgfx_startlz[1] = static_cast<byte*>(W_CacheLumpName("STRTLZ2",  PU_STATIC));
        rawgfx_startbot   = static_cast<byte*>(W_CacheLumpName("STRTBOT",  PU_STATIC));
...
void D_DoomMain (void)
{
    ...
    {
        char msgbuf[80];
        char *serial  = static_cast<char*>(W_CacheLumpName("SERIAL", PU_CACHE));
...
//    W_PrintDirectory();
    ...
    if (p)
    {
        char *uc_filename = _strdup(myargv[p + 1]);
    ...
    startskill = static_cast<skill_t>((crispy->defaultskill + SKILL_HNTR) % NUM_SKILLS);
