...
static void InitHashTable(void)
{
    ...
    hash_table = static_cast<deh_substitution_t**>(Z_Malloc(sizeof(deh_substitution_t *) * hash_table_length, PU_STATIC, NULL));
...
static boolean ValidFormatReplacement(const char *original, const char *replacement)
{
    ...
    for (;;)
    {
        ...
        else if (!ValidArgumentReplacement(static_cast<format_arg_t>(argtype1), static_cast<format_arg_t>(argtype2)))
        {
