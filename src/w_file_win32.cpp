...
static void MapFile(win32_wad_file_t *wad, const char *filename)
{
    ...
    wad->wad.mapped = static_cast<byte*>(MapViewOfFile(wad->handle_map, FILE_MAP_COPY, 0, 0, 0));
...
static wad_file_t *W_Win32_OpenFile(const char *path)
{
    ...
    result = static_cast<win32_wad_file_t*>(Z_Malloc(sizeof(win32_wad_file_t), PU_STATIC, 0));
