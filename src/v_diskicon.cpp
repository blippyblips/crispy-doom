...
static void SaveDiskData(const char *disk_lump, int xoffs, int yoffs)
{
    ...
    tmpscreen = static_cast<pixel_t*>(Z_Malloc(SCREENWIDTH * SCREENHEIGHT * sizeof(*tmpscreen),
                         PU_STATIC, NULL));
    ...
    disk_data = static_cast<pixel_t*>(Z_Malloc(LOADING_DISK_W * LOADING_DISK_H * sizeof(*disk_data),
                         PU_STATIC, NULL));
...
void V_EnableLoadingDisk(const char *lump_name, int xoffs, int yoffs)
{
    ...
    saved_background = static_cast<pixel_t*>(Z_Malloc(LOADING_DISK_W * LOADING_DISK_H * sizeof(*saved_background), PU_STATIC, NULL));
