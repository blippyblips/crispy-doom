...
void R_GenerateComposite (int texnum)
{
    ...
    block = static_cast<byte*>(static_cast<byte*>(Z_Malloc (texturecompositesize[texnum],
		      PU_STATIC, 
		      &texturecomposite[texnum])));
    // [crispy] memory block for opaque textures
    block2 = static_cast<byte*>(Z_Malloc (texture->width * texture->height,
		      PU_STATIC,
    ...
    source = static_cast<byte*>(I_Realloc(NULL, texture->height)); // temporary column
...
//  if (texture->patchcount > 1 && texture->height < 256)
	...
	{
	    int pat = patch->patch;
	    const patch_t *realpatch = static_cast<patch_t*>(W_CacheLumpNum(pat, PU_CACHE));
...
static void GenerateTextureHashTable(void)
{
    ...
    textures_hashtable = static_cast<texture_t**>(Z_Malloc(sizeof(texture_t *) * numtextures, PU_STATIC, 0));
...
void R_InitTextures (void)
{
    ...
    {
	texturelumps[i].maptex = static_cast<int*>(W_CacheLumpNum(texturelumps[i].lumpnum, PU_STATIC));
    ...
    texturecolumnlump = static_cast<short**>(Z_Malloc (numtextures * sizeof(*texturecolumnlump), PU_STATIC, 0));
    texturecolumnofs = static_cast<unsigned int**>(Z_Malloc (numtextures * sizeof(*texturecolumnofs), PU_STATIC, 0));
    texturecolumnofs2 = static_cast<unsigned int**>(Z_Malloc (numtextures * sizeof(*texturecolumnofs2), PU_STATIC, 0));
    texturecomposite = static_cast<byte**>(Z_Malloc (numtextures * sizeof(*texturecomposite), PU_STATIC, 0));
    texturecomposite2 = static_cast<byte**>(Z_Malloc (numtextures * sizeof(*texturecomposite2), PU_STATIC, 0));
    ...
    texturewidth = static_cast<int*>(Z_Malloc (numtextures * sizeof(*texturewidth), PU_STATIC, 0));
    textureheight = static_cast<fixed_t*>(Z_Malloc (numtextures * sizeof(*textureheight), PU_STATIC, 0));
    texturebrightmap = static_cast<const byte**>(Z_Malloc (numtextures * sizeof(*texturebrightmap), PU_STATIC, 0));
	...
	texturecolumnlump[i] = static_cast<short*>(Z_Malloc (texture->width*sizeof(**texturecolumnlump), PU_STATIC,0));
	texturecolumnofs[i] = static_cast<unsigned int*>(Z_Malloc (texture->width*sizeof(**texturecolumnofs), PU_STATIC,0));
	texturecolumnofs2[i] = static_cast<unsigned int*>(Z_Malloc (texture->width*sizeof(**texturecolumnofs2), PU_STATIC,0));
...
void R_InitFlats (void)
{
    ...
    // Create translation table for global animation.
    flattranslation = static_cast<int*>(Z_Malloc ((numflats+1)*sizeof(*flattranslation), PU_STATIC, 0));
...
void R_InitSpriteLumps (void)
{
    ...
    spritewidth = static_cast<fixed_t*>(Z_Malloc (numspritelumps*sizeof(*spritewidth), PU_STATIC, 0));
    spriteoffset = static_cast<fixed_t*>(Z_Malloc (numspritelumps*sizeof(*spriteoffset), PU_STATIC, 0));
    spritetopoffset = static_cast<fixed_t*>(Z_Malloc (numspritelumps*sizeof(*spritetopoffset), PU_STATIC, 0));
...
static void R_InitTranMap()
{
    ...
    if (lump != -1 && W_LumpLength(lump) == 256*256)
    {
        ...
        tranmap = static_cast<byte*>(W_CacheLumpNum(lump, PU_STATIC));
	...
	unsigned char *playpal = static_cast<unsigned char*>(W_CacheLumpName("PLAYPAL", PU_STATIC));

	tranmap = static_cast<byte*>(Z_Malloc(256*256, PU_STATIC, 0));
	{
...
void R_InitColormaps (void)
{
    ...
    colormaps = static_cast<lighttable_t*>(W_CacheLumpNum(lump, PU_STATIC));
#else
	int c, i, j = 0;
	byte r, g, b;
...
{
	byte *playpal = static_cast<byte*>(W_CacheLumpName("PLAYPAL", PU_STATIC));
	char c[3];
	int i, j;
	boolean keepgray = false;

	if (!crstr)
	    crstr = static_cast<char**>(I_Realloc(NULL, CRMAX * sizeof(*crstr)));
	...
	if (i >= 0)
	{
	    cr[CR_RED2GREEN] = static_cast<byte*>(W_CacheLumpNum(i, PU_STATIC));
	}
	...
	if (i >= 0)
	{
	    cr[CR_RED2BLUE] = static_cast<byte*>(W_CacheLumpNum(i, PU_STATIC));
	}
}
