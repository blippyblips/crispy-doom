...
{
    V_DrawPatchFullScreen(static_cast<patch_t*>(W_CacheLumpName(pagename, PU_CACHE)), crispy->fliplevels);
}
...
static char *GetGameName(const char *gamename)
{
    ...
    for (i=0; i<arrlen(banners); ++i)
    {
        ...
        if (deh_sub != banners[i])
        {
            ...
            deh_gamename = new char[gamename_size];
...
static void D_Endoom(void)
{
    ...
    endoom = static_cast<byte*>(W_CacheLumpName(DEH_String("ENDOOM"), PU_STATIC));
