...
{
    int format = mapformat_t::MFMT_DOOMBSP;
    byte *nodes = NULL;
    int b;
    ...
    return static_cast<mapformat_t>(format);
}
...
void P_LoadNodes_ZDBSP (int lump, boolean compressed)
{
    ...
    data = static_cast<byte*>(W_CacheLumpNum(lump, PU_LEVEL));
    ...
    {
	newvertarray = static_cast<vertex_t*>(Z_Malloc((orgVerts + newVerts) * sizeof(vertex_t), PU_LEVEL, 0));
