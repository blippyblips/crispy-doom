...
void M_DrawSave(void)
{
    ...
    V_DrawPatchDirect(SaveDef_x, SaveDef_y, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_SAVEG"), PU_CACHE)));
    for (i = 0;i < load_end; i++)
    {
	M_DrawSaveLoadBorder(LoadDef.x,LoadDef.y+LINEHEIGHT*i);
	M_WriteText(LoadDef.x,LoadDef.y+LINEHEIGHT*i,savegamestrings[i]);
...
void M_DrawReadThisCommercial(void)
{
    ...
    V_DrawPatchFullScreen(static_cast<patch_t*>(W_CacheLumpName(DEH_String("HELP"), PU_CACHE)), false);
}
...
static void M_DrawMouse(void)
{
    ...
    V_DrawPatchDirect (60, LoadDef_y, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_MSENS"), PU_CACHE)));
...
static void M_DrawCrispnessBackground(void)
{
	...
	{
		src = static_cast<pixel_t*>(W_CacheLumpNum(lump, PU_CACHE));
	}
	dest = I_VideoBuffer;
...
void M_Drawer (void)
{
    ...
    V_DrawPatchDirect(x + SKULLXOFF, currentMenu->y - 5 + itemOn*LINEHEIGHT,
		      static_cast<patch_t*>(W_CacheLumpName(DEH_String(skullName[whichSkull]), PU_CACHE)));
}
...
void M_Init (void)
{
        ...
        if (crispy->havenerve)
        {
            ...
            if (i != -1 && j != -1)
            {
                ...
                if (W_IsIWADLump(lumpinfo[i]) && W_IsIWADLump(lumpinfo[j]))
                {
                    ...
                    pi = static_cast<patch_t*>(W_CacheLumpNum(i, PU_CACHE));
                    pj = static_cast<patch_t*>(W_CacheLumpNum(j, PU_CACHE));
	...
	patchl = static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_LOADG"), PU_CACHE));
	patchs = static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_SAVEG"), PU_CACHE));
	patchm = static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_LSLEFT"), PU_CACHE));
