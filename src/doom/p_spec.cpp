...
void P_InitPicAnims (void)
{
    ...
    if (from_lump)
    {
	animdefs = static_cast<animdef_t*>(W_CacheLumpName("ANIMATED", PU_STATIC));
    }
    else
    {
...
void P_PlayerInSpecialSector (player_t* player)
{
	...
	if (showMessages && crispy->secretmessage && player == &players[consoleplayer])
	{
	    ...
	    player->centermessage = const_cast<char*>((crispy->secretmessage == SECRETMESSAGE_COUNT) ? str_count : HUSTR_SECRETFOUND);
	    if (sfx_id != -1)
