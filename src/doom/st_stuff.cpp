...
static inline void ST_DrawGibbedPlayerSprites (void)
{
	...
	patch = static_cast<patch_t*>(W_CacheLumpNum(sprframe->lump[0] + firstspritelump, PU_CACHE));
...
void ST_drawWidgets(boolean refresh)
{
	...
	if (plyr->readyweapon == wp_fist && plyr->powers[pw_strength])
	{
		...
		patch = static_cast<patch_t*>(W_CacheLumpNum(lump, PU_CACHE));
...
void ST_loadData(void)
{
	...
	keys[i] = static_cast<patch_t*>((lumpnum != -1) ? W_CacheLumpNum(lumpnum, PU_STATIC) : keys[i-3]);
    }
}
