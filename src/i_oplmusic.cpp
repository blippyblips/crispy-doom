...
static boolean LoadInstrumentTable(void)
{
    ...
    lump = static_cast<byte*>(W_CacheLumpName(DEH_String("genmidi"), PU_STATIC));
...
static void LoadOperatorData(int op, genmidi_op_t *data, boolean max_level, unsigned int *volume)
{
    int level;
...
static void I_OPL_PlaySong(void *handle, boolean looping)
{
    ...
    file = static_cast<midi_file_t*>(handle);

    // Allocate track data.

    tracks = static_cast<opl_track_data_t*>(malloc(MIDI_NumTracks(file) * sizeof(opl_track_data_t)));
