...
static void AllocateBuffer(const unsigned int size)
{
    ...
    buffer.data = static_cast<byte*>(I_Realloc(buffer.data, buffer.size));
}
...
static void *I_WIN_RegisterSong(void *data, int len)
{
    ...
    if (IsMid(static_cast<byte*>(data), len))
    {
    ...
    song.tracks = static_cast<win_midi_track_t*>(calloc(song.num_tracks, sizeof(win_midi_track_t)));
    for (i = 0; i < song.num_tracks; ++i)
    {
