...
static void CrispyDrawStats (void)
{
    ...
    if (!height || !coord_x || !coord_w)
    {
        ...
        const patch_t *const p = static_cast<const patch_t*>(W_CacheLumpNum(FontABaseLump + 'A' - 33, PU_CACHE));
...
//
    ...
    if (paused && !MenuActive && !askforquit)
    {
        if (!netgame)
        {
            V_DrawPatch(160, (viewwindowy >> crispy->hires) + 5, static_cast<patch_t*>(W_CacheLumpName(DEH_String("PAUSED"), PU_CACHE)));
        }
        else
        {
            V_DrawPatch(160, 70, static_cast<patch_t*>(W_CacheLumpName(DEH_String("PAUSED"), PU_CACHE)));
        }
    }
...
void D_PageDrawer(void)
{
    ...
    if (demosequence == 1)
    {
        V_DrawPatch(4, 160, static_cast<patch_t*>(W_CacheLumpName(DEH_String("ADVISOR"), PU_CACHE)));
    }
...
void initStartup(void)
{
    ...
    loading = static_cast<byte*>(W_CacheLumpName(DEH_String("LOADING"), PU_CACHE));
...
static void D_Endoom(void)
{
    ...
    endoom_data = static_cast<byte*>(W_CacheLumpName(DEH_String("ENDTEXT"), PU_STATIC));
