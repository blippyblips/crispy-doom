...
void IN_DrawStatBack(void)
{
    ...
    src = static_cast<byte*>(W_CacheLumpName(DEH_String("FLOOR16"), PU_CACHE));
    dest = I_VideoBuffer;
...
void IN_DrawDMStats(void)
{
        ...
        for (i = 0; i < MAXPLAYERS; i++)
        {
            if (playeringame[i])
            {
                ...
                V_DrawShadowedPatch(((xpos << FRACBITS) +
                                    ...
                                    static_cast<patch_t*>(W_CacheLumpNum(patchFaceDeadBase + i, PU_CACHE)));
            }
        }
        sounds = 0;
        return;
    ...
    for (i = 0; i < MAXPLAYERS; i++)
    {
        if (playeringame[i])
        {
            ...
            {
                V_DrawAltTLPatch(40, ypos, static_cast<patch_t*>(W_CacheLumpNum(patchFaceOkayBase + i, PU_CACHE)));
                V_DrawAltTLPatch(xpos, 18, static_cast<patch_t*>(W_CacheLumpNum(patchFaceDeadBase + i, PU_CACHE)));
            }
            kpos = 86;
            for (j = 0; j < MAXPLAYERS; j++)
            {
