...
static void DrawMainMenu(void)
{
    ...
    frame = (MenuTime / 3) % 18;
    V_DrawPatch(88, 0, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_HTIC"), PU_CACHE)));
...
static void DrawSaveLoadBottomLine(const Menu_t *menu)
{
    ...
    if (!width)
    {
        const patch_t *const p = static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_FSLOT"), PU_CACHE));
        width = SHORT(p->width);
    }
...
static void DrawFileSlots(Menu_t * menu)
{
    ...
    for (i = 0; i < SAVES_PER_PAGE; i++)
    {
        V_DrawPatch(x, y, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_FSLOT"), PU_CACHE)));
        if (SlotStatus[i])
        {
...
static void DrawSlider(Menu_t * menu, int item, int width, int slot)
{
    ...
    V_DrawPatch(x - 32, y, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_SLDLT"), PU_CACHE)));
    for (x2 = x, count = width; count--; x2 += 8)
    {
        V_DrawPatch(x2, y, static_cast<patch_t*>(W_CacheLumpName(DEH_String(count & 1 ? "M_SLDMD1" : "M_SLDMD2"), PU_CACHE)));
    }
    V_DrawPatch(x2, y, static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_SLDRT"), PU_CACHE)));
    ...
    V_DrawPatch(x + 4 + slot * 8, y + 7,
                static_cast<patch_t*>(W_CacheLumpName(DEH_String("M_SLDKB"), PU_CACHE)));
}
...
static void M_DrawCrispnessBackground(void)
{
    ...
    if (gamemode == shareware)
    {
        src = static_cast<byte*>(W_CacheLumpName(DEH_String("FLOOR04"), PU_CACHE));
    }
    else
    {
        src = static_cast<byte*>(W_CacheLumpName(DEH_String("FLAT513"), PU_CACHE));
    }
    dest = I_VideoBuffer;
