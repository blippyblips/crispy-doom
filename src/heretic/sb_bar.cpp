...
void SB_Init(void)
{
    ...
    PatchLTFACE = static_cast<patch_t*>(W_CacheLumpName(DEH_String("LTFACE"), PU_STATIC));
    PatchRTFACE = static_cast<patch_t*>(W_CacheLumpName(DEH_String("RTFACE"), PU_STATIC));
    PatchBARBACK = static_cast<patch_t*>(W_CacheLumpName(DEH_String("BARBACK"), PU_STATIC));
    PatchINVBAR = static_cast<patch_t*>(W_CacheLumpName(DEH_String("INVBAR"), PU_STATIC));
    PatchCHAIN = static_cast<patch_t*>(W_CacheLumpName(DEH_String("CHAIN"), PU_STATIC));
    if (deathmatch)
    {
        PatchSTATBAR = static_cast<patch_t*>(W_CacheLumpName(DEH_String("STATBAR"), PU_STATIC));
    }
    else
    {
        PatchSTATBAR = static_cast<patch_t*>(W_CacheLumpName(DEH_String("LIFEBAR"), PU_STATIC));
    }
    if (!netgame)
    {                           // single player game uses red life gem
        PatchLIFEGEM = static_cast<patch_t*>(W_CacheLumpName(DEH_String("LIFEGEM2"), PU_STATIC));
    }
    else
    {
        PatchLIFEGEM = static_cast<patch_t*>(W_CacheLumpNum(W_GetNumForName(DEH_String("LIFEGEM0")) + consoleplayer, PU_STATIC));
    }
    PatchLTFCTOP = static_cast<patch_t*>(W_CacheLumpName(DEH_String("LTFCTOP"), PU_STATIC));
    PatchRTFCTOP = static_cast<patch_t*>(W_CacheLumpName(DEH_String("RTFCTOP"), PU_STATIC));
    PatchSELECTBOX = static_cast<patch_t*>(W_CacheLumpName(DEH_String("SELECTBOX"), PU_STATIC));
    PatchINVLFGEM1 = static_cast<patch_t*>(W_CacheLumpName(DEH_String("INVGEML1"), PU_STATIC));
    PatchINVLFGEM2 = static_cast<patch_t*>(W_CacheLumpName(DEH_String("INVGEML2"), PU_STATIC));
    PatchINVRTGEM1 = static_cast<patch_t*>(W_CacheLumpName(DEH_String("INVGEMR1"), PU_STATIC));
    PatchINVRTGEM2 = static_cast<patch_t*>(W_CacheLumpName(DEH_String("INVGEMR2"), PU_STATIC));
    PatchBLACKSQ = static_cast<patch_t*>(W_CacheLumpName(DEH_String("BLACKSQ"), PU_STATIC));
    PatchARMCLEAR = static_cast<patch_t*>(W_CacheLumpName(DEH_String("ARMCLEAR"), PU_STATIC));
    PatchCHAINBACK = static_cast<patch_t*>(W_CacheLumpName(DEH_String("CHAINBACK"), PU_STATIC));
    startLump = W_GetNumForName(DEH_String("IN0"));
    for (i = 0; i < 10; i++)
    {
        PatchINumbers[i] = static_cast<patch_t*>(W_CacheLumpNum(startLump + i, PU_STATIC));
    }
    PatchNEGATIVE = static_cast<patch_t*>(W_CacheLumpName(DEH_String("NEGNUM"), PU_STATIC));
...
static void DrINumber(signed int val, int x, int y)
{
    ...
    if (val < 0)
    {
        if (val < -9)
        {
            V_DrawPatch(x + 1, y + 1, static_cast<patch_t*>(W_CacheLumpName(DEH_String("LAME"), PU_CACHE)));
        }
        else
        {
            val = -val;
...
void SB_Drawer(void)
{
    ...
    else
    {
        if (SB_state == -1)
        {
            ...
            {
                V_DrawPatch(16, 167,
                            static_cast<patch_t*>(W_CacheLumpName(DEH_String("GOD1"), PU_CACHE)));
                V_DrawPatch(287, 167,
                            static_cast<patch_t*>(W_CacheLumpName(DEH_String("GOD2"), PU_CACHE)));
            }
            oldhealth = -1;
        }
        DrawCommonBar();
    ...
    if (CPlayer->powers[pw_flight])
    {
            ...
            || !(CPlayer->powers[pw_flight] & 16))
        {
            ...
            else
            {
                if (!hitCenterFrame && (frame != 15 && frame != 0))
                {
                    V_DrawPatch(spinfly_x, 17, static_cast<patch_t*>(W_CacheLumpNum(spinflylump + frame, PU_CACHE)));
                    hitCenterFrame = false;
                }
                else
                {
                    V_DrawPatch(spinfly_x, 17, static_cast<patch_t*>(W_CacheLumpNum(spinflylump + 15, PU_CACHE)));
                    hitCenterFrame = true;
                }
            }
...
void DrawMainBar(void)
{
             ...
             || oldartiCount != CPlayer->inventory[inv_ptr].count)
    {
        ...
        {
            V_DrawPatch(179, 160,
                        static_cast<patch_t*>(W_CacheLumpName(DEH_String(patcharti[CPlayer->readyArtifact]), PU_CACHE)));
    ...
    if (oldkeys != playerkeys)
    {
        ...
        {
            V_DrawPatch(153, 164, static_cast<patch_t*>(W_CacheLumpName(DEH_String("ykeyicon"), PU_CACHE)));
        }
        if (CPlayer->keys[key_green])
        {
            V_DrawPatch(153, 172, static_cast<patch_t*>(W_CacheLumpName(DEH_String("gkeyicon"), PU_CACHE)));
        }
        if (CPlayer->keys[key_blue])
        {
            V_DrawPatch(153, 180, static_cast<patch_t*>(W_CacheLumpName(DEH_String("bkeyicon"), PU_CACHE)));
        }
        oldkeys = playerkeys;
        UpdateState |= I_STATBAR;
    }
    ...
    if (oldammo != temp || oldweapon != CPlayer->readyweapon)
    {
        ...
        if (temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
        {
            ...
            V_DrawPatch(111, 172,
                        static_cast<patch_t*>(W_CacheLumpName(DEH_String(ammopic[CPlayer->readyweapon - 1]), PU_CACHE)));
        }
        oldammo = temp;
...
void DrawFullScreenStuff(void)
{
    ...
    if (!inventory)
    {
        if (CPlayer->readyArtifact > 0)
        {
            ...
            V_DrawAltTLPatch(286, 170, static_cast<patch_t*>(W_CacheLumpName(DEH_String("ARTIBOX"), PU_CACHE)));
    ...
    else
    {
        ...
        for (i = 0; i < 7; i++)
        {
            V_DrawAltTLPatch(50 + i * 31, 168,
                          static_cast<patch_t*>(W_CacheLumpName(DEH_String("ARTIBOX"), PU_CACHE)));
            if (CPlayer->inventorySlotNum > x + i
                && CPlayer->inventory[x + i].type != arti_none)
            {
                patch = DEH_String(patcharti[CPlayer->inventory[x + i].type]);
...
static weapontype_t PickBestWeapon(player_t *player)
{
    ...
    for (i = wp_mace; i >= wp_goldwand; i--)
    {
        ...
        {
            return static_cast<weapontype_t>(i);
        }
    }
...
static void CheatAddRemoveWpnFunc(player_t *player, Cheat_t *cheat)
{
    ...
    {
        P_GiveWeapon(player, static_cast<weapontype_t>(w));
