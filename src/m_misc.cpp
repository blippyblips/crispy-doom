...
static wchar_t *ConvertMultiByteToWide(const char *str, UINT code_page)
{
    ...
    wstr = static_cast<wchar_t*>(malloc(sizeof(wchar_t) * wlen));
...
static char *ConvertWideToMultiByte(const wchar_t *wstr, UINT code_page)
{
    ...
    str = static_cast<char*>(malloc(sizeof(char) * len));
...
char *M_getenv(const char *name)
{
    ...
    env_vars = static_cast<env_var_t*>(I_Realloc(env_vars, (num_vars + 1) * sizeof(*env_vars)));
...
int M_ReadFile(const char *name, byte **buffer)
{
    ...
    buf = static_cast<byte*>(Z_Malloc (length + 1, PU_STATIC, NULL));
