...
static void AddWADFileName(const char *filename)
{
    ...
    wad_filenames = static_cast<char**>(I_Realloc(wad_filenames, (i + 2) * sizeof(*wad_filenames)));
...
int W_LumpDump (const char *lumpname)
{
    ...
    lump_p = static_cast<char*>(malloc(lumpinfo[i]->size));
    W_ReadLump(i, lump_p);
