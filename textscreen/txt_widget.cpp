...
txt_callback_table_t *TXT_NewCallbackTable(void)
{
    ...
    table = static_cast<txt_callback_table_t*>(malloc(sizeof(txt_callback_table_t)));
                       ...
                       void *user_data)
{
    ...
    table->callbacks = static_cast<txt_callback_t*>(realloc(table->callbacks, sizeof(txt_callback_t) * (table->num_callbacks + 1)));
