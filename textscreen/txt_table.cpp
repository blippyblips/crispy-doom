...
static void TXT_CalcTableSize(TXT_UNCAST_ARG(table))
{
    ...
    row_heights = static_cast<unsigned int*>(malloc(sizeof(int) * rows));
    column_widths = static_cast<unsigned int*>(malloc(sizeof(int) * table->columns));
...
void TXT_AddWidget(TXT_UNCAST_ARG(table), TXT_UNCAST_ARG(widget))
{
    ...
    table->widgets = static_cast<txt_widget_t**>(realloc(table->widgets, sizeof(txt_widget_t *) * (table->num_widgets + 1)));
...
txt_table_t *TXT_NewTable(int columns)
{
    ...
    table = static_cast<txt_table_t*>(malloc(sizeof(txt_table_t)));
